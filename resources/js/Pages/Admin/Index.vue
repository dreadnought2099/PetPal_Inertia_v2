<template>
    <div class="container mx-auto p-4">
      ...
      >
        <div class="modal-content bg-white p-6 rounded-lg shadow-xl text-center max-w-sm w-full">
          <p class="mb-4 text-lg text-gray-800" v-html="modal.message"></p>
  ...
  function confirmRoleChange(user) {
    ...
    showModal(
      `Change <span style="color: #49b451;">${user.name}'s</span> role to <span style="color: #49b451;">${capitalize(selectedRole)}</span>?`,
      () => {
        router.post(route('admin.changeRole', user.id), { role: selectedRole });
      }
    );
  }
  </script>
